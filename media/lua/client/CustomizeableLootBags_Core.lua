...
-----------------------
function CustomizeableLootBags.setLootBag(bag, shouldRemove, recorder)
    if CustomizeableLootBags.isBag(bag) then
        ...
        recorder:setName(bag:getDisplayName())
        recorder:getModData()['LootBagData'] = {}
        ...
        if showTip then
			recorder:setTooltip("Spawns: "..strList)
		end
...
function CustomizeableLootBags.invContext(player, context, items)
	...
	local useAll = SandboxVars.CustomizeableLootBags.CanUseAllIcons or false
		...
		if item then
			if isElevated then
				if CustomizeableLootBags.isBag(item) then
					local recorder = item:getContainer():FindAndReturn('Base.LootBagRecorder')
					if recorder then
						local optTip = context:addOptionOnTop("Set Loot Bag: "..tostring(item:getDisplayName()), worldobjects, function()
							...
							CustomizeableLootBags.setLootBag(item, isShouldDel, recorder)
